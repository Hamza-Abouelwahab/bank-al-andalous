...
import { ArrowDownToLine, CheckCircle, ShieldCheck, X, Zap } from 'lucide-react';
...
export default function Deposit() {
    ...
    const confirmDeposit = () => {
        post('/deposit', {
            preserveScroll: true,
            onSuccess: () => { setReview(false); setSuccess(true); },
        });
    };
        ...
        <>
                        ...
                        <div className="lg:col-span-8 order-2 lg:order-1">
                            <div className="fintech-card rounded-3xl border border-[#EDE8E0] bg-white p-8 shadow-sm dark:border-[#2A2520] dark:bg-[#1A1714]">
                                <form onSubmit={submit} className="space-y-7">
                                    ...
                                    <div>
                                        <label className="mb-3 block text-xs font-bold uppercase tracking-widest text-[#1f1a17] dark:text-white">Deposit Amount</label>
                                        <div className="relative group">
                                    ...
                                    <div>
                                        <label className="mb-3 block text-xs font-bold uppercase tracking-widest text-[#1f1a17] dark:text-white">Deposit Source</label>
                                        <div className="grid grid-cols-2 gap-3">
                                            {SOURCES.map((s) => (
                                                <button key={s.value} type="button"
                                                    onClick={() => setData('source', s.value)}
                                                    className={`group relative rounded-2xl border p-4 text-sm font-semibold transition-all duration-200 overflow-hidden ${data.source === s.value
                                                        ? 'border-orange-600 bg-orange-600 dark:bg-orange-600/50 dark:border-orange-600/20 border-orange-600/50 text-white shadow-lg '
                                                        : 'border-orange-200 text-[#1f1a17] hover:border-orange-300 hover:bg-orange-600/20 dark:border-[#2A2520] dark:text-white dark:hover:bg-orange-600/50'
                                                    }`}>
                                        ...
                                        {errors.source && <p className="mt-2.5 text-xs font-medium text-red-600 dark:text-red-400">{errors.source}</p>}
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <input value={data.description}
                                            ...
                                            className="h-12 w-full rounded-2xl border dark:border-[#2A2520] focus:dark:border-orange-600 border-orange-200 bg-white px-4 text-sm text-[#1f1a17] outline-none transition-all duration-200 focus:border-orange-600  dark:bg-[#1f1a17] dark:text-white" />
                                    </div>

                                    {/* Submit Button - Premium */}
                                    <button type="submit" disabled={!isValid || processing}
                                        className="w-full rounded-2xl bg-orange-600 py-4 text-sm font-bold text-white shadow-sm transition-all duration-300 hover:bg-orange-700 cursor-pointer disabled:opacity-50">
                        ...
                        <div className="lg:col-span-4 order-1 lg:order-2 space-y-5">
                                    ...
                                    {parsed > 0 && (
                                        <div className="border-t border-white/15 pt-3">
                                            <div className="flex items-center justify-between">
                                                <span className="text-xs font-medium text-white/60">After deposit</span>
                                                <span className="text-lg font-bold text-orange-200">{fmt(newBalance)}</span>
                                            </div>
                                        </div>
                                    )}
                                </div>
                            ...
                            <div className="fintech-card rounded-3xl border border-[#EDE8E0] bg-white p-6 shadow-sm transition-all duration-200 hover:shadow-md dark:border-[#2A2520] dark:bg-[#1A1714]">
                                ...
                                <div className="space-y-3 text-sm">
                                    {[
                                        ...
                                        ['Processing fee', 'Free'],
                                        ['Total credited', parsed ? `${parsed.toLocaleString()} MAD` : '—'],
                                    ].map(([k, v]) => (
                            ...
                            <div className="fintech-card group rounded-3xl border border-orange-200/60 bg-gradient-to-br from-orange-50 to-[#f8f6f1] p-5 transition-all duration-200 hover:shadow-md dark:border-[#7a2800]/40 dark:from-orange-900/10 dark:to-[#7a2800]/5">
                                <div className="flex items-start gap-3">
                                    ...
                                    <div>
                                        <p className="text-sm font-bold text-[#7a2800] dark:text-orange-300">Secure Deposit</p>
                                        <p className="mt-1 text-xs text-[#7a2800]/80 dark:text-orange-400/70 leading-relaxed">
                                            Protected by 256-bit bank-grade encryption for every deposit.
                                        </p>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div className="relative z-10 w-full max-w-sm rounded-3xl border border-[#EDE8E0] bg-white p-7 shadow-2xl dark:border-[#2A2520] dark:bg-[#1A1714] animate-scale-in">
                        ...
                        <div className="mb-6 flex items-start justify-between">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h2 className="text-lg font-bold text-[#1f1a17] dark:text-white">Confirm Deposit</h2>
                        ...
                        <div className="mb-6 space-y-3 rounded-2xl bg-gradient-to-br from-[#f8f6f1] to-orange-50/50 p-5 dark:from-orange-900/10 dark:to-[#7a2800]/5">
                            {[['Amount', `${parsed.toLocaleString()} MAD`], ['Source', data.source], ['New Balance', fmt(newBalance)]].map(([k, v]) => (
            ...
            {success && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-fade-in">
                    ...
                    <div className="relative z-10 w-full max-w-sm rounded-3xl border border-[#EDE8E0] bg-white p-8 text-center shadow-2xl dark:border-[#2A2520] dark:bg-[#1A1714] animate-scale-in">
                        ...
                        <h2 className="text-xl font-extrabold text-[#1f1a17] dark:text-white">Deposit Successful!</h2>
                        <p className="mt-2 text-sm text-[#1f1a17]/70 dark:text-white/70">Your funds have been added to your account.</p>
